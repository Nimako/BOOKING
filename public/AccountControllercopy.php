...
class AccountController extends Controller
{
    ...
    public function SignUpFireBase(Request $request)
    {
        ...
        $duplicatePhoneNum = UserAccount::where('PhoneNum',$request->PhoneNum)->first();
        $duplicateEmail    = UserAccount::where('Email',$request->Email)->first();
        $fireBaseUserID    = UserAccount::where('FireBaseUserID',$request->FireBaseUserID)->first();
        $fireBaseKey       = UserAccount::where('FireBaseKey',$request->FireBaseKey)->first();

        if(!empty($duplicateEmail)){

            return response()->json([ 'statusCode' => 500, 'message' => "Email Already Exist"]);

        }elseif(!empty($duplicatePhoneNum) || !empty($duplicatePhoneNum->PhoneNum)){

            return response()->json(['statusCode' => 500, 'message' => "Phone Number Already Exist"]);

        }elseif(!empty($fireBaseUserID)){

            return response()->json([ 'statusCode' => 500, 'message' => "Firebase UserID Already Exist"]);

        }/*elseif(!empty($fireBaseKey)){ 

            return response()->json([ 'statusCode' => 500, 'message' => "Firebase key Already Exist"]);
            ...
            $UserAccount->DisplayName     = $request->DisplayName;
            $UserAccount->DateBirth       = $request->DateBirth;
            $UserAccount->PhoneNum        = $request->PhoneNum;
            $UserAccount->Country         = $request->Country;
            $UserAccount->City            = $request->City;
            $UserAccount->Region          = $request->Region;
            $UserAccount->ProfileImage    = $request->ProfileImage;
            $UserAccount->Provider        = $request->Provider;
            //$UserAccount->FireBaseKey     = $request->FireBaseKey;  
            $UserAccount->FireBaseUserID  = $request->FireBaseUserID;                                                                                                                                                                                                                                               
            if($UserAccount->save()) {
