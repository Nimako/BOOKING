...
class WebPropertyController extends Controller
{
   ...
   public function pendingProperties(){
       ...
       $data['list'] = Property::where(['status'=>PENDING])->get();

       return view("properties/pendingProperties", $data);
   }


   public function approvedProperties(){
      
      $data['list'] = Property::where(['status'=>3])->get();

      return view("properties/approvedProperties", $data);
  }
   ...
   public function propertyDetail($id){
      ...
      if(!empty($id)){
         ...
         $generalImages = array();
         //$searchedProperty = Property::with('details')->where(['uuid' => $id])->first();
         $searchedProperty   = Property::with('hoteldetails', 'OtherHotelDetails')->where(['uuid' => $id])->first();

         if(!empty($searchedProperty)){

         if($searchedProperty->property_type_id==3){
            ...
            $data['info'] =  $searchedProperty;

            return view("properties/hotelDetails", $data);

         }elseif($searchedProperty->property_type_id==1){
           ...
           $searchedProperty = Property::with('details')->where(['uuid' => $id])->first();
            ...
            return view("properties/apartmentDetails", $data);
         }
       }
      }
   }
   ...
   public function ChangePropertyStatus(Request $request){
      ...
      $query = Property::where('uuid', $request->id)->update(['status' => $request->status]);

      return redirect()->route('propertyDetail', ['id' => $request->id]);
